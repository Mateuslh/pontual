...
class DeliveryRepository {
    ...
    suspend fun startDelivery(id: Int): Result<Delivery> {
        return withContext(Dispatchers.IO) {
            try {
    ...
    suspend fun completeDelivery(id: Int): Result<Delivery> {
        return withContext(Dispatchers.IO) {
            try {
    ...
    suspend fun updateDeliveryStatus(id: Int, pointId: Int, status: String): Result<Delivery> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.updateDeliveryStatus(id, pointId, status)
                if (response.isSuccessful) {
                    response.body()?.let { delivery ->
                        Result.success(delivery)
